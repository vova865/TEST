...
RSpec.describe ExchangeIt::Account do
  ...
  context 'when performing money wihtdrawal' do
    ...
    describe '#withdraw' do
      ...
      it 'does not allow to withdraw a sum that is too large' do
        expect { john.withdraw(130) }.to raise_error(ExchangeIt::NotEnoughFunds, /Available: 100/)
      end

      it 'does not allow to withdraw a negative sum' do
        expect { john.withdraw(-30) }.to raise_error(ExchangeIt::IncorrectSum, /must be positive/)
      end
